...
# и выводит их в виде списка - массива. Элементы могут быть не попорядку и не рядом
# и их может быть несколько как только программа нахрдит одно решение она
...
def two_summ (array_inp: list, total: int):
    ...
    fin_list_fin = [] # финальный список элекментов значения

    list_position = {}

    for i in range(0, len(array_inp)): # словарь где запоминаем позицю элемт
    ...
    return sum_t, fin_list_fin, sorted(fin_list2)
